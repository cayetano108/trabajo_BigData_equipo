...
empty_bar <- 3
to_add <- data.frame( matrix(NA, empty_bar*nlevels(campeones$nationality.x), ncol(campeones)) )
colnames(to_add) <- colnames(campeones)
to_add$nationality.x <- rep(levels(campeones$nationality.x), each=empty_bar)
campeones <- cbind(id=id, campeones)
campeones <- campeones %>% arrange(nationality.x)
campeones$id <- seq(1, nrow(campeones))
...
p <- ggplot(campeones, aes(x=as.factor(year), y=total_campeonatos, fill=nationality.x)) + geom_bar(aes(x=as.factor(id), y=total_campeonatos, fill=nationality.x), stat="identity", alpha=0.5) +# Add a val=100/75/50/25 lines. I do it at the beginning to make sur barplots are OVER it.
  geom_segment(data=grid_campeones, aes(x = end, y = 8, xend = start, yend = 8), colour = "grey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_campeones, aes(x = end, y = 6, xend = start, yend = 6), colour = "grey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_campeones, aes(x = end, y = 4, xend = start, yend = 4), colour = "grey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_campeones, aes(x = end, y = 2, xend = start, yend = 2), colour = "grey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  ...
  #geom_segment(data=base_campeones, aes(x = start, y = -10, xend = end, yend = -5), colour = "black", alpha=0.8, size=0.6 , inherit.aes = FALSE ) #+
 # geom_text(data=base_campeones, aes(x = title, y = -18, label=nationality.x), hjust=c(1,1,1,0), colour = "black", alpha=0.8, size=4, fontface="bold", inherit.aes = FALSE)
