...
pilotos <- rio::import(file = "./datosf1/drivers.csv")
resultados <- rio::import(file = "./datosf1/results.csv")
...
mas_victorias <- resultados %>% filter(position == "1") %>% group_by(driverId) %>% mutate(n_victorias = sum(n())) %>% distinct(n_victorias) 

#aqui fusiono con el df de pilotos para que aparezca el nombre y no sólo el ID del piloto en cuestion
victorias_con_nombre <- full_join(mas_victorias, pilotos, c ("driverId" = "driverId")) %>% select(driverId, driverRef, n_victorias) 
#-------------------------------------------------------------------------------------
...
escuderias <- rio::import(file = "./datosf1/constructors.csv")
escuderias <- rio::import(file = "./datosf1/constructor_standings.csv")
result_escuderias <- rio::import(file = "./datosf1/constructor_results.csv")
#escuderiasesp <- escuderias %>% filter(nationality == "Spanish") #escuderias españolas
#----------------------------------------------------------------------------------------


circuitos <- rio::import(file = "./datosf1/circuits.csv")
tiempos <- rio::import(file = "./datosf1/lap_times.csv")
carreras <- rio::import(file = "./datosf1/races.csv")
